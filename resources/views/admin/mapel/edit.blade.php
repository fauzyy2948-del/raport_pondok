    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.mapel.update', $mapel) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label">Kategori</label><select name="kategori" class="form-select"><option value="diniyah" {{ old('kategori',$mapel->kategori) === 'diniyah' ? 'selected' : '' }}>Diniyah</option><option value="umum" {{ old('kategori',$mapel->kategori) === 'umum' ? 'selected' : '' }}>Umum</option></select></div>
