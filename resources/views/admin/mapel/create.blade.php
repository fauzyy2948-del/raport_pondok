    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.mapel.store') }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label">Kategori</label><select name="kategori" class="form-select"><option value="diniyah" {{ old('kategori') === 'diniyah' ? 'selected' : '' }}>Diniyah</option><option value="umum" {{ old('kategori') === 'umum' ? 'selected' : '' }}>Umum</option><option value="muatan_lokal" {{ old('kategori') === 'muatan_lokal' ? 'selected' : '' }}>Muatan Lokal</option></select></div>
