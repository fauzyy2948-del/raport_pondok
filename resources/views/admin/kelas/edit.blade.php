...
<div class="card" style="max-width:500px;">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.kelas.update', $kelas) }}">
            ...
            <div class="mb-3"><label class="form-label">Wali Kelas</label><select name="ustadz_id" class="form-select"><option value="">-- Pilih Wali Kelas --</option>@foreach($ustadzList as $u)<option value="{{ $u->id }}" {{ old('ustadz_id', $kelas->ustadz_id) == $u->id ? 'selected' : '' }}>{{ $u->nama_lengkap }}</option>@endforeach</select></div>
