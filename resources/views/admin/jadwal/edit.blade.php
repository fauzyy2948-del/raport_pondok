                        ...
                        <div class="col-md-6">
                            ...
                            <select name="ustadz_id" class="form-select @error('ustadz_id') is-invalid @enderror" required>
                                ...
                                @foreach($ustadzs as $u)
                                    <option value="{{ $u->id }}" {{ old('ustadz_id', $jadwal->ustadz_id) == $u->id ? 'selected' : '' }}>
                                        {{ $u->nama_lengkap }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="jam_select" class="form-select @error('jam_mulai') is-invalid @enderror @error('jam_selesai') is-invalid @enderror" required>
                                ...
                                @foreach(\App\Models\Jadwal::listJam() as $time => $label)
                                    @php
                                        ...
                                        $dbStart = substr($jadwal->jam_mulai, 0, 5);
                                        $dbEnd = substr($jadwal->jam_selesai, 0, 5);
                                        $selected = (old('jam_mulai', $dbStart) == $start && old('jam_selesai', $dbEnd) == $end) ? 'selected' : '';
                                    @endphp
                                    <option value="{{ $time }}" {{ $selected }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            <input type="hidden" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai', substr($jadwal->jam_mulai, 0, 5)) }}">
                            <input type="hidden" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai', substr($jadwal->jam_selesai, 0, 5)) }}">
